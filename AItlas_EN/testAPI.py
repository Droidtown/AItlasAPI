...
def runEthik(inputSTR, level="lv1", userDefinedDICT={}):
    payload = {"input_str": inputSTR,
               "level": level,
               "user_defined_dict_file": userDefinedDICT}
    respond = requests.post("{}/Ethik/API/".format(G_url), json=payload)
    resultDICT = respond.json()
    print(respond)
    #pprint(resultDICT)
    return resultDICT

def runEthikBulk(inputLIST, level="lv1", userDefinedDICT={}):
    payload = {"input_list": inputLIST,
               "level": level,
               "user_defined_dict_file": userDefinedDICT}
    respond = requests.post("{}/Ethik/BulkAPI/".format(G_url), json=payload)
    resultDICT = respond.json()
    print(respond)
    #pprint(resultDICT)
    return resultDICT
...
if __name__ == "__main__":
    ...
    print("runEthik()")
    pprint(runEthik(inputLIST[0]))

    print("runEthikBulk()")
    pprint(runEthikBulk(inputLIST))
    ...
    #run list of str through EthikBulk
