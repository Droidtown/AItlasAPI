...
def debugInfo(inputSTR, utterance):
    if DEBUG_Person_Secure:
        print("[Person_Secure] {} ===> {}".format(inputSTR, utterance))

def getResult(inputSTR, utterance, args, resultDICT):
    debugInfo(inputSTR, utterance)
    if utterance == "[He] [secured] the [empire] [with] a [buffer] [region] [of] [client] [states]":
